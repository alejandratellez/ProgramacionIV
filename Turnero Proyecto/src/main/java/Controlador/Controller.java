...
////*****************CRUD DE PUESTO ATENCION***************************
	 ...
	 * EntityManager em = getEntityManager();
	 * 
	 * // TODO Auto-generated method stub String priori; String mensaje;
	 * this.prioridad.setPrioridad(prioridad); priori =
	 * this.prioridad.getPrioridad(); System.out.println(priori);
	 * 
	 * if (priori.length() == 0) { mensaje = "Por favor ingrese una Prioridad";
	 * return mensaje;
	 * 
	 * } else if (consultarPrioridad(priori).getPrioridad().equals(priori)) {
	 * 
	 * String prioridad2 = consultarPrioridad(priori).getPrioridad(); System.out
	 * .println(prioridad + "El id es" + consultarPrioridad(priori).getId() +
	 * "La Prioridad es duplicado"); mensaje = "La Prioridad esta duplicado"; return
	 * mensaje; } else { em.getTransaction().begin(); this.prioridad =
	 * em.merge(this.prioridad); em.getTransaction().commit(); em.close(); mensaje =
	 * "Se ingreso correctamente la Prioridad";
	 * 
	 * // nodoPrioridad = modelo.insertarServicio(servicio); Lista modelo = new
	 * Lista(); modelo.mostrarElementos(); return mensaje; }
	 * 
	 * }
	 * 
	 * 
	 * public String guardarServicio(String servicio) { String mensaje;
	 * EntityManager em = getEntityManager();
	 * 
	 ...
	 * 
	 * if (nodoServicio.getServ().length() == 0) { mensaje =
	 * "Por favor ingrese un nombre de servicio"; return mensaje; } else if
	 * (consultarServicioNombre(servicio).getServ().equals(servicio)) { String
	 * servprueba; servprueba = consultarServicioNombre(servicio).getServ();
	 * System.out.println( servprueba + "El id es" +
	 * consultarServicioNombre(servicio).getId() + "El Servicio es duplicado");
	 * mensaje = "El servicio esta duplicado"; return mensaje; } else {
	 * em.getTransaction().begin(); nodoServicio = em.merge(nodoServicio);
	 * em.getTransaction().commit(); em.close(); mensaje =
	 * "Se ingreso correctamente el servicio";
	 * 
	 * return mensaje; } }
	 * 
	 * 
	 * public String guardarCliente(String nombre, int documento, String prioridad,
	 * String servicio) { System.out.println("entra"); String mensaje;
	 * 
	 * // TODO Auto-generated method stub Servicio servicios = new
	 * Servicio(servicio); System.out.println("entra"); Prioridad prioridades = new
	 * Prioridad(prioridad); System.out.println("entra"); // String nombres =
	 * nombre;
	 * 
	 * EntityManager em = getEntityManager(); Cliente nodoCliente = new Cliente();
	 * nodoCliente.setId(documento); nodoCliente.setNombre(nombre);
	 * 
	 * servicios = consultarServicioNombre(servicio); System.out.println("entra");
	 * int dato; dato = servicios.getId();
	 * 
	 * nodoCliente.getServicio().add(consultarServicio(dato));
	 * nodoCliente.getServicio().add(consultarServicio(dato));
	 * System.out.println("entra ojo"); prioridades = consultarPrioridad(prioridad);
	 * 
	 * int datoPrioridad; datoPrioridad = prioridades.getId();
	 * nodoCliente.getServicio().add(consultarServicio(datoPrioridad));
	 * nodoCliente.getServicio().add(consultarServicio(datoPrioridad));
	 * 
	 * System.out.println("entra2"); em.getTransaction().begin();
	 * 
	 * nodoCliente = em.merge(nodoCliente); em.getTransaction().commit();
	 * em.close(); mensaje = "su Turno es x";
	 * 
	 * return mensaje; }
	 */
